...
export class ArchiveManagerService extends TypertRemoteService {
  ...
  async pruneArchivedWorkspaceSnapshots() {
    try {
      ...
      if (archived.size > 0 && persisted.size === 0) {
        this.ctx.logger?.debug?.('dsh-archive-manager: persistence list is not ready; preserving archive index')
        setTimeout(() => this.pruneArchivedWorkspaceSnapshots().catch(() => {}), 2500)
        return
      }
